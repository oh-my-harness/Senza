...
1. BM25 index：关注文档频率如何计算，以及 `search` 如何做长度归一化与排序。
...
3. MemoryStore：注意它只定义 descriptor、`upsert` 和 `delete`，并明确不作为 KnowledgeSource 使用。
...
5. Session Recall 入口、Projector 和 History Recall Plugin：入口列出完整装配条件；Projector 建投影；Plugin 在上下文变换阶段读取并注入。
