...
1. 文件工具的 Plugin 聚合入口共享同一个 snapshot store，并集中注册各文件工具。
2. Senza 的 [`src/lib.rs`](../../src/lib.rs) 暴露 `create_os_env()` 与
   `create_fs_tools_plugin()`；OS env 直接调用真实文件系统和宿主 shell。
3. [`pyrules.rs`](../../src/runtime/pyrules.rs) 把 Python RuleChain 变成
   ...
   rate-limit 的通过证据。
4. [`pysafety.rs`](../../src/strategy/pysafety.rs) 是 Python Plugin 工厂；具体分发逻辑只在工具名为
   `bash`、`read`、`write`、`edit` 时进入相应检查。
5. 默认命令检测规则与已知限制、静态 `..` 检查和可选的 canonicalize 检查共同构成路径与命令安全检查。继续沿调用点核对会发现：标准 Senza run
...
6. `OsEnvSandbox` 被描述为“without security isolation”，这是不能把 OS env 宣传成强沙箱的
   最直接证据。
...
python -m academy.scenarios describe safety.rules_approval
```
