...
| 按步重跑 | `WorkflowEngine.restore_from_step(...)` | 运行引擎的截断与重定位 | 保留目标之前历史，从已执行过的目标 step 重驱动 |
...
从 Python 侧理解 Workflow 的关键入口，可按“定义 → 执行 → 持久化 → 恢复 → 外部事件”依次对照：

1. `Workflow`、`Step` 与 `Edge` 定义流程拓扑；`WorkflowState`、`StepRecord` 与 `Transition`
   是运行期状态对象，运行前会校验 entry、step id 与 edge 引用。
2. `StepTransitionJudge` 负责 step 结果到 `Transition` 的判断接口；`create_judge(callback)` 可用
   Python 回调构造它。
3. 运行循环由 `WorkflowEngine` 驱动，负责 step history 落盘、`restore_from_step()`、边界暂停、
   恢复和取消。其语义明确：context 不随历史回滚，截断取 target 的最近一次出现位置。
4. `.with_task_store(dir)` 启用 `TaskStore`（JSONL 实现）持久化流程、状态、上下文与 checkpoint；
   持久化格式是恢复输入，因此加载后仍需校验 Workflow。
5. Senza 的 [`pyworkflow.rs`](../../src/runtime/pyworkflow.rs) 暴露 Python 构造、Executor/Judge 注册、
   restore、pause/resume/cancel 和 step history；`parse_transition()` 也可核对 `pause:<reason>`。
6. `create_event_channel()` + `.with_external_tool()` 提供事件等待 Tool，实现 event/timeout/abort
   三路等待；[`pyeventstream.rs`](../../src/core/pyeventstream.rs) 展示 Python channel 工厂为何使用
   `task_store=None`——进程崩溃后重建 EventStream 仍是业务层职责。
