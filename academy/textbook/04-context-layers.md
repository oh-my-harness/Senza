...
2. 当存在可见 Skill 时，构建器会自动注册 `SkillReadTool`；run 开始时会注入目录元数据。
3. 目录 XML、正文读取、路径约束和 256 KiB 截断边界由 Runtime 的 Skill 读取工具实现。
4. [`pystatuspanel.rs`](../../src/strategy/pystatuspanel.rs) 只是 Senza 工厂；真正的能力组合
   （`todo_write`、`transform_context` 与 `after_tool_call`）在 Runtime 的 StatusPanel 插件内，
   上下文末尾注入逻辑由其 Hook 负责。
5. Python 的手工入口位于 [`pyharness.rs`](../../src/core/pyharness.rs)；自动阈值、
   `before_compact` 决策、熔断和 Session 写入由 Runtime 的压缩流程实现。
