# 第 2 章：14 个 Hook——在固定生命周期边界扩展 Agent Core

> 成熟度：**stable**。Runtime 当前公开 14 个 Hook 类型，并为同类 Hook 定义了确定的组合语义。
> Academy recorded 图谱用于核对全部契约；当前 Python live example 只覆盖其中六类，不代表单次
> 在线运行触发了全部 Hook。
...
答案是：Agent Core 预先声明 **14 个 Hook 类型**。Hook 的开放性体现在"同一个固定槽位可以注册
...
1. 准确列出 Runtime 的 14 个 Hook 类型，并说明它们的职责；
...
Runtime 的 `HarnessHooks` 保存 14 个 Hook 向量。Builder 按注册顺序追加实现；构建后的 Harness 在
运行时把同类实现包装成 `Composite*`，再由 Core 在固定边界调用。
...
| `on_abort` | `abort()` 中止信号触发时 | 置标志、cancel token、释放资源 | 全部同步执行，纯副作用 |
...
这条故事没有触发全部 14 个 Hook：压缩是条件分支，Tool Hook 只有发生 Tool Call 才出现，
`should_stop` 只在模型自然停止时查询。14 表示**契约类型数**，不是每次 run 的固定回调次数。

## 源码导读：从槽位到控制流

### 1. 类型层：Hook 能看到什么、能返回什么

Runtime 定义各 Hook trait、上下文和决策类型。例如 `BeforeToolCallCtx` 提供 Tool 名、参数、turn
index 和 run 引用；返回值只能是 Allow、Modify 或 Deny。这个类型边界决定 Plugin 能做什么，而不
是 Python callback 自由返回任意对象。

### 2. Harness 层：14 个固定向量

`HarnessHooks` 明确列出 14 个字段。`none()` 构造全部为空的默认值。新增第 15 个生命周期类型需要
修改 Runtime 契约与控制流；普通 Plugin 不能在运行时“声明一个新槽位”让 Core 自动调用。

### 3. 组合层：同类 Hook 怎样合成

构建后的 Harness 把同类 Hook 的多个实现包装成单个 Composite 实现，再由 Core 在固定边界调用。
组合语义因 Hook 类型而异：

- `before_tool_call`：遇到首个非 Allow 立即返回；
- `after_tool_call`：维护当前结果，让 Replace 串联；
- `transform_context`：逐个传递 `AgentContext`；
- `prepare_next_turn`：按字段保留最后一个非空值；
- `should_stop`：所有实现都执行，最后做 OR；
- `final_answer_validator`：首个拒绝短路。

这些组合语义有回归测试直接验证。例如 after-tool 测试验证两个替换都保留，should-stop 测试验证
早期 `true` 不会跳过后续 Hook。

### 4. 运行层：Core 在哪里交出控制权

Runtime 的 Core 循环负责 Provider、Context、Tool 与停止决策；run/turn 通知、Session 写入和
Harness 状态由驱动层负责。Tool 前后 Hook 不是散落在 callback 内，而是在构建 LoopConfig 时用
`HookedTool` 统一包住所有当前 Tool。

`before_run` 和自动压缩的入口在 `run()` 中：自动压缩先处理，随后才是 `before_run`，最后进入
loop。

### 5. Python 层：callback 怎样变成 Hook

[`pyhooks.rs`](../../src/core/pyhooks.rs) 将 Python callback 包装为 14 种 Hook trait。以
`before_tool_call` 为例，Senza 把 ctx 转为 Python dict，再把字符串或 dict 返回值解析为 Rust
决策；callback 异常会 fail-closed 为 Deny，而不是悄悄放行。

`HookKind` 只包含这 14 类，`push_into()` 根据类型把实现放入对应向量。因此 Python API 虽然写起来
...
输出把主路径、压缩分支和答案提交分支放在同一张图中，以便一次核对 14 个名称。请把它当作
...
测试确保 14 个名称各出现一次，并抽查四种不同组合语义：Tool 前短路、Context 链式变换、Stop
...
`before_turn`、`after_turn`、`before_tool_call`、`after_tool_call`、`after_run` 和 `on_abort`。检查 Tool 前后计数是否对称，
并明确记录：这只能证明六类 Hook 的当前 Python 装配链路，不能外推为 14 类全部在线覆盖。
...
不可以。Core 只会调用 `HarnessHooks` 中声明的 14 类。Plugin 可以选择一个或多个现有槽位并注册
...
- Hook 类型固定为 14，数量不得与 Plugin 或 Senza 内置策略工厂数量混淆；
- Python 当前暴露 14 类包装，但每个 live example 的覆盖面可能更小；
...
14 个 Hook 把 Harness 的可变策略安放在稳定 Agent Core 周围。真正决定可组合性的，不只是“有一个
