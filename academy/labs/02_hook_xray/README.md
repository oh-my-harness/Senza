...
验证与纠正。书给出"为什么"；Runtime 把这些职责落到 Agent Core 预先定义的 14 个
生命周期边界。Hook 的开放性是“可在固定边界注册多个实现”，不是 Plugin 可以选择源码
任意一行插入逻辑。

权威定义见 Runtime 的 `HarnessHooks` trait；多 Hook 组合语义由 composite 层统一调度。
...
| `on_abort` | `abort()` 中止信号触发时 | 全部同步执行，纯副作用（置标志/cancel token） |
...
当前实际证明的是 `before_turn`、`after_turn`、`before_tool_call`、`after_tool_call`、
`after_run`、`on_abort` 六个边界及工具前后对称性；本课不会把它描述为"单次 live 覆盖 14 个 Hook"。
...
它们被放在一份轨迹里是为了核对 14 类契约，并不声称每次 run 都会触发全部 Hook，
也不声称图谱顺序替代 Runtime 源码中的实际控制流。

## 能力边界

- 14 个位置由 Agent Core 定义；Plugin 只能选择已声明的槽位；
