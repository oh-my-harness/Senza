"""33 — Steering & Multi-turn: guide a running agent and manage conversation flow.

Mirrors runtime example `11_steering` (原仓库根 examples/agent/11_steering.py).
Demonstrates:
  ...
  - ``last_response()`` — the assistant's last response text
...
Run:
  source ~/.omp_llm_env && python live-tests/examples/33_steering.py
"""
...
def main() -> None:
    print("=== 33: Steering & Multi-turn ===\n")

    harness = make_example_harness(
        lambda b: b.system_prompt(
            "You are a research assistant. Answer clearly and concisely."
        ).max_tokens(512)
    )
    ...
    print(f"  Response: {harness.last_response()}")
