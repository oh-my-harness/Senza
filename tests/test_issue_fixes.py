...
def test_create_openai_provider_with_chat_path():
    ...
    provider = senza.create_openai_provider(api_key="test-key", chat_path="/chat/completions")
    assert provider is not None


def test_create_openai_provider_with_thinking_scheme():
    """create_openai_provider accepts thinking_scheme (#59)."""
    provider = senza.create_openai_provider(api_key="test-key", thinking_scheme="reasoning_effort")
    assert provider is not None


def test_create_openai_provider_thinking_toggle():
    provider = senza.create_openai_provider(api_key="test-key", thinking_scheme="thinking_toggle")
    assert provider is not None
...
def test_create_openai_provider_invalid_thinking_scheme():
    ...
    with pytest.raises(ValueError):
        senza.create_openai_provider(api_key="test-key", thinking_scheme="bogus")


def test_create_openai_provider_strips_trailing_v1():
    """base_url ending in /v1 doesn't crash — auto-stripped (#59)."""
    provider = senza.create_openai_provider(api_key="test-key", base_url="http://localhost:8080/v1")
    assert provider is not None
...
def test_prompt_and_collect_propagates_llm_error():
    ...
    """
    provider = senza.create_openai_provider(api_key="bad-key", base_url="http://127.0.0.1:1")
    harness = (
